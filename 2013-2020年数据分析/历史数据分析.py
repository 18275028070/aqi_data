...
# 数据清洗
def get_data():
    df = pd.read_csv('./data/AQI历史数据2.csv')
    print(df.head())  # 查看表格

    df1 = pd.read_csv('./data/city.csv', encoding='GB18030')
    print(df1.head())  # 查看表格。此表为坐标表格，本项目后续改进时增加地图显示城市空气质量
    ...
    print(df.head())
    print(df.shape, df.describe())  # 查看数据数量
    print(df.info())
...
# 数据分析预测阶段
# def data_forecast():
#     df = pd.read_excel("./data/chengdu.xlsx", sheet_name="Sheet1").head()
#     print(df)
#     print(df.shape)
#     print(df.describe())
#     print(df.info)
#     print(df.isnull().sum())  # 查看是否存在缺失值
#
#     # 构建线性回归模型
#     model = LinearRegression()
#     # 定义特征和目标变量
#     X = df[['aqi', 'pm2_5', 'pm10', 'so2', 'no2', 'co', 'o3']]
#     y = df['aqi']
#     # 拟合模型并预测
#     model.fit(X, y)
...
#     print('模型R2值：', model.score(X, y))
#     print('模型均方误差：', np.mean((y_pred - y) ** 2))
...
def data_analysis():
    ...
    plt.rcParams['font.sans-serif'] = ['KaiTi', 'SimHei', 'FangSong']  # 显示中文标签
    ...
    name_list = list(df["cityname"].drop_duplicates())
    value_list = list(df["aqi"])
    # 对应的城市数据
    data = [z for z in zip(name_list, value_list)]

    print([z for z in zip(keys, values)])
    ...
    geo.render('2013-2020全国主要城市空气质量图.html')
    ...
    sns.regplot(x='pm2_5', y='aqi', data=df_over_polluted)
    plt.show()
    ...
    # 这里只关注轻度污染以上时的污染物情况
    df_over_polluted_city = df_top10_polluted_city[df_top10_polluted_city['aqi'] >= 100][['aqi', 'pm2_5', 'pm10',
                                                                                          'so2', 'no2', 'co', 'o3']]
    sns.regplot(x='pm2_5', y='aqi', data=df_over_polluted_city)
    # plt.show()
    sns.regplot(x='pm10', y='aqi', data=df_over_polluted_city)
    # plt.show()
    sns.regplot(x='so2', y='aqi', data=df_over_polluted_city)
    # plt.show()
    sns.regplot(x='no2', y='aqi', data=df_over_polluted_city)
    # plt.show()
    sns.regplot(x='co', y='aqi', data=df_over_polluted_city)
    # plt.show()
    sns.regplot(x='o3', y='aqi', data=df_over_polluted_city)
    # plt.show()

    plt.figure(figsize=(15, 5))
    sns.heatmap(df_over_polluted_city.corr(), vmax=1, square=False, annot=True, linewidth=1)
    plt.yticks(rotation=0)
    # plt.show()
    ...
    plt.legend('AQI')
    plt.grid(linestyle=':', color='w')
    print(df.groupby('season').aqi.mean().sort_values())
    plt.show()

    """
        问题1.4
                2013年-2020年全国整体空气质量如何变化?
    """
    ...
    plt.style.use('dark_background')
    plt.ylim(50)
    plt.title('2013年~2020年全国空气质量1~12月对比')
    plt.show()
    ...
    """
    df_sc = df[df['省份'] == '四川省']
    df_sc_pollutant = df_sc[df_sc['aqi'] >= 100][['aqi', 'pm2_5', 'pm10', 'so2', 'no2', 'co', 'o3']]
    sns.regplot(x='pm2_5', y='aqi', data=df_sc_pollutant)
    # plt.show()
    sns.regplot(x='pm10', y='aqi', data=df_sc_pollutant)
    # plt.show()
    sns.regplot(x='so2', y='aqi', data=df_sc_pollutant)
    # plt.show()
    sns.regplot(x='no2', y='aqi', data=df_sc_pollutant)
    # plt.show()
    sns.regplot(x='co', y='aqi', data=df_sc_pollutant)
    # plt.show()
    sns.regplot(x='o3', y='aqi', data=df_sc_pollutant)
    # plt.show()
    plt.figure(figsize=(15, 5))
    sns.heatmap(df_sc_pollutant.corr(), vmax=1, square=False, annot=True, linewidth=1)
    plt.yticks(rotation=0)
    # plt.show()

    print('四川省的城市数量为：', df_sc.groupby('cityname').aqi.mean().shape[0])
    print(df_sc.groupby('cityname').aqi.mean().sort_values().tail(3))  # 得出污染较严重的3个城市

    """
        问题2.2：全省哪些城市污染较严重？（AQI作为判断依据）
    """
    print('四川省的城市数量为：', df_sc.groupby('cityname').aqi.mean().shape[0])
    df_sc.groupby('cityname').aqi.mean().sort_values().tail(3)  # 得出污染较严重的3个城市

    """
        问题2.3：全省哪个季节的污染最严重？
    """
    pd.DataFrame(df_sc.groupby('season').aqi.mean().sort_values()).plot.barh(figsize=(15, 10))
    plt.title('四川省不同季节空气质量情况')
    plt.xlabel('AQI')
    plt.ylabel('季节')
    plt.xlim(50)
    plt.legend('AQI')
    ...
    """
        问题2.4：四川省空气质量在全国的排名
    """
    print(df_sc[['省份', '所属省份空气质量排名']].head(1))

    """
        3.全市范围(成都市)：
            问题3.1：成都市的主要污染物是什么？
    """
    df_cd = df[df['cityname'] == '成都']
    df_cd_pollutant = df_cd[df_cd['aqi'] >= 100][['aqi', 'pm2_5', 'pm10', 'so2', 'no2', 'co', 'o3']]
    sns.regplot(x='pm2_5', y='aqi', data=df_cd_pollutant)
    # plt.show()
    sns.regplot(x='pm10', y='aqi', data=df_cd_pollutant)
    # plt.show()
    sns.regplot(x='so2', y='aqi', data=df_cd_pollutant)
    # plt.show()
    sns.regplot(x='no2', y='aqi', data=df_cd_pollutant)
    # plt.show()
    sns.regplot(x='co', y='aqi', data=df_cd_pollutant)
    # plt.show()
    sns.regplot(x='o3', y='aqi', data=df_cd_pollutant)
    # plt.show()
    plt.figure(figsize=(15, 5))
    sns.heatmap(df_cd_pollutant.corr(), vmax=1, square=False, annot=True, linewidth=1)
    plt.yticks(rotation=0)
    # plt.show()
    ...
    """
        问题3.2：成都空气质量在全国的排名?
    """
    print(df_cd[['cityname', '城市空气质量全国排名']].head(1))
    df_cd.groupby('空气质量').time.count().plot.bar(figsize=(10, 5))
    plt.xticks(rotation=0)
    plt.ylabel('次数')
    plt.title('2013-2020成都空气质量次数图')
    # plt.show()
    print('平均AQI为：%.f\n\n' % (df_cd.aqi.mean()), df_cd.groupby('空气质量').time.count())

    """
         问题3.3：成都哪个季节的污染最严重？？
    """
    pd.DataFrame(df_cd.groupby('season').aqi.mean().sort_values()).plot.barh(figsize=(15, 10))
    plt.title('成都不同季节空气质量情况')
    plt.xlabel('AQI')
    plt.ylabel('季节')
    plt.xlim(40)
    plt.legend('AQI')
